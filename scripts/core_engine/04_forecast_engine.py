...
# Revolve Balance -> Interest Revenue/Cost of Funds; Outstanding Balance ->
# Fee Revenue/Charge Offs; Active Accounts -> Other Revenue/Servicing), so
# the signal propagates automatically without being applied a second time to
# any of those -- doing so WOULD double-count.
#
# NOTE: Cost of Funds is keyed to Revolve Balance, not Outstanding Balance,
# specifically so it moves with Interest Revenue (same basis) rather than
# with the out-of-phase Outstanding Balance/NTV pair -- otherwise Interest
# Revenue's seasonal swing has no cost-side offset and lands almost entirely
# in the thin Gross Profit/Contribution Profit residual.
SEASONAL_LINE_ITEMS = {"Net Transaction Volume", "Revolve Balance", "Outstanding Balance", "In-Month Active Accounts"}


def load_all():
    actuals = pd.read_parquet(OUT_DIR / "parquet" / "clean_actuals_grained.parquet")
    dev_factors = pd.read_csv(OUT_DIR / "csv" / "curve_dev_factors.csv")
    seeds = pd.read_csv(OUT_DIR / "csv" / "curve_qsb0_seeds.csv")
    rate_curves = pd.read_csv(OUT_DIR / "csv" / "curve_rate_curves.csv")
    acq_rates = pd.read_csv(OUT_DIR / "csv" / "curve_acquisition_rates.csv")
    pooled = set(pd.read_csv(OUT_DIR / "csv" / "pooled_merchants.csv")["pooled_merchant"])
    classification = pd.read_csv(DATA_DIR / "line_item_classification.csv")
    pooled_tail_factors = pd.read_csv(OUT_DIR / "csv" / "curve_pooled_tail_factors.csv")
    pooled_tail_rates = pd.read_csv(OUT_DIR / "csv" / "curve_pooled_tail_rates.csv")
    ...
    seasonal_csv = pd.read_csv(OUT_DIR / "csv" / "curve_seasonal_index.csv")
    seasonal_index = {
...
def main():
    ...
    new_accounts_frontbook.to_csv(OUT_DIR / "csv" / "frontbook_new_accounts.csv", index=False)
    ...
    combined.to_parquet(OUT_DIR / "parquet" / "combined_actuals_forecast.parquet", index=False)
    print(f"\nWrote combined actuals+forecast table -> {OUT_DIR / 'parquet' / 'combined_actuals_forecast.parquet'}")
