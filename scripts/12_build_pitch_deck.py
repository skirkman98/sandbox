...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .stat {{ flex: 1; min-width: 150px; background: var(--page); border: 1px solid var(--grid); border-radius: 10px; padding: 0.9rem 1.1rem; }}
  section.slide.alt .stat {{ background: #fff; }}
  ...
  .bug-card {{ background: var(--page); border-left: 3px solid var(--red); border-radius: 0 8px 8px 0; padding: 0.85rem 1.1rem; margin-bottom: 0.9rem; }}
  section.slide.alt .bug-card {{ background: #fff; }}
...
<nav class="top-nav">
  ...
  <span><a href="dashboard.html">Executive dashboard</a><a href="narrative_report.html">Narrative walkthrough</a></span>
</nav>
...
<section class="slide">
  ...
  <svg class="diagram" viewBox="0 0 900 220" role="img" aria-label="Architecture diagram: drivers feed a rate curve library, which multiplies out to P&amp;L dollar lines">
    ...
    <g font-family="inherit" font-size="13.5">
      <rect x="20" y="70" width="190" height="80" rx="10" fill="{PALETTE['blue']}" opacity="0.12" stroke="{PALETTE['blue']}"/>
      <text x="115" y="100" text-anchor="middle" font-weight="700" fill="{PALETTE['blue']}">Drivers</text>
      <text x="115" y="120" text-anchor="middle" fill="{PALETTE['ink2']}" font-size="11.5">New Accounts, Balances,</text>
      <text x="115" y="135" text-anchor="middle" fill="{PALETTE['ink2']}" font-size="11.5">Volume &mdash; chain-ladder curves</text>

      <line x1="210" y1="110" x2="290" y2="110" stroke="{PALETTE['muted']}" stroke-width="1.5" marker-end="url(#arrow)"/>

      <rect x="300" y="70" width="220" height="80" rx="10" fill="{PALETTE['orange']}" opacity="0.12" stroke="{PALETTE['orange']}"/>
      <text x="410" y="100" text-anchor="middle" font-weight="700" fill="{PALETTE['orange']}">Rate curve library</text>
      <text x="410" y="120" text-anchor="middle" fill="{PALETTE['ink2']}" font-size="11.5">$ line &divide; driver, by cohort age</text>
      <text x="410" y="135" text-anchor="middle" fill="{PALETTE['ink2']}" font-size="11.5">(loss rate, yield, interchange rate...)</text>

      <line x1="520" y1="110" x2="600" y2="110" stroke="{PALETTE['muted']}" stroke-width="1.5" marker-end="url(#arrow)"/>

      <rect x="610" y="70" width="270" height="80" rx="10" fill="{PALETTE['aqua']}" opacity="0.12" stroke="{PALETTE['aqua']}"/>
      <text x="745" y="100" text-anchor="middle" font-weight="700" fill="{PALETTE['aqua']}">P&amp;L dollar lines</text>
      <text x="745" y="120" text-anchor="middle" fill="{PALETTE['ink2']}" font-size="11.5">rate &times; forecasted driver</text>
      <text x="745" y="135" text-anchor="middle" fill="{PALETTE['ink2']}" font-size="11.5">&rarr; Gross Revenue, Cost of Sales, ...</text>

      <text x="450" y="20" text-anchor="middle" fill="{PALETTE['muted']}" font-size="12">One mechanism handles a genuinely flat rate (yield, ~6.05%/quarter) and a genuinely curving one</text>
      <text x="450" y="36" text-anchor="middle" fill="{PALETTE['muted']}" font-size="12">(loss rate, rising from 0% toward a plateau near quarter 9&ndash;13) without hand-coding which is which.</text>
    </g>
  </svg>
  <div class="stat-row">
  ...
  <div class="two-col">
    ...
    <svg class="diagram" viewBox="0 0 380 240" role="img" aria-label="Backbook cohorts anchored to last actual and grown forward; frontbook cohorts seeded from a growth trend and grown on the same curve">
      <g font-family="inherit" font-size="12">
        <text x="10" y="20" font-weight="700" fill="{PALETTE['blue']}">Backbook</text>
        <line x1="10" y1="60" x2="130" y2="60" stroke="{PALETTE['blue']}" stroke-width="2"/>
        <circle cx="130" cy="60" r="4" fill="{PALETTE['blue']}"/>
        <line x1="130" y1="60" x2="230" y2="45" stroke="{PALETTE['blue']}" stroke-width="2" stroke-dasharray="4,3"/>
        <text x="10" y="80" fill="{PALETTE['ink2']}" font-size="11">actual history</text>
        <text x="140" y="42" fill="{PALETTE['ink2']}" font-size="11">curve-forecast</text>

        <text x="10" y="130" font-weight="700" fill="{PALETTE['orange']}">Frontbook</text>
        <circle cx="30" cy="170" r="4" fill="{PALETTE['orange']}"/>
        <line x1="30" y1="170" x2="230" y2="150" stroke="{PALETTE['orange']}" stroke-width="2" stroke-dasharray="4,3"/>
        <text x="10" y="190" fill="{PALETTE['ink2']}" font-size="11">seeded from</text>
        <text x="10" y="203" fill="{PALETTE['ink2']}" font-size="11">growth trend</text>
        <text x="140" y="143" fill="{PALETTE['ink2']}" font-size="11">same curve library</text>
      </g>
    </svg>
  </div>
</section>
