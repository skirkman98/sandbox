...
def main():
    data = json.loads((OUT_DIR / "json" / "dashboard_data.json").read_text())
    data_json = json.dumps(data, separators=(",", ":"))

    html = HTML_TEMPLATE.replace("__DASHBOARD_DATA__", data_json)
    out_path = OUT_DIR / "html" / "dashboard.html"
...
<header class="page-head">
  ...
  <nav class="doc-links">
    <a href="unified_narrative.html">Overview &rarr;</a>
...
function renderSmallLineChart(containerId, series, fmt, color) {
  ...
  const W = 320, H = 130, ML = 8, MR = 8, MT = 16, MB = 20;
  const plotW = W - ML - MR, plotH = H - MT - MB;
  const vals = finite.map(s => s.value);
  // These are all rate/ratio metrics that typically live in a narrow band
  // well away from zero (e.g. Active Rate 57%-71%) -- unlike the $ charts
  // elsewhere on this page, forcing the axis to include 0 would compress
  // the real, meaningful variation into a sliver at the top of the chart
  // and make genuine quarter-to-quarter seasonality invisible. Auto-scaled
  // to the actual data range instead, with the max/min labeled explicitly
  // (see the text labels below) so a narrower-than-zero axis doesn't read
  // as more dramatic than the real numbers -- a legitimate, standard
  // line-chart practice as long as the scale is disclosed, not hidden.
  const dataMax = Math.max(...vals), dataMin = Math.min(...vals);
  const pad = (dataMax - dataMin) * 0.18 || Math.abs(dataMax) * 0.1 || 1;
  ...
  svg.appendChild(svgEl("line", { class: "axis-line", x1: ML, x2: W - MR, y1: MT + plotH, y2: MT + plotH }));
  // Explicit max/min labels -- the disclosure that makes a non-zero axis honest.
  svg.appendChild(Object.assign(svgEl("text", { x: ML, y: MT - 4, "font-size": "9" }), { textContent: fmt(dataMax) }));
  svg.appendChild(Object.assign(svgEl("text", { x: ML, y: MT + plotH - 2, "font-size": "9" }), { textContent: fmt(dataMin) }));
...
function renderCompareChartSvg(seriesList, blended, metric) {
  ...
  // Bar (additive $) metrics anchor to zero -- meaningful magnitude context,
  // and some (Contribution Profit) genuinely cross zero. Line (rate/yield)
  // metrics auto-scale to the actual data range instead: these typically
  // live in a narrow band away from zero (e.g. Revolve Rate 20%-60%), and
  // forcing a zero baseline would crush real quarter-to-quarter movement
  // into an unreadable sliver -- the gridline labels below (metric.fmt(v))
  // keep a non-zero axis honest, same reasoning as the small KPI charts.
  let yMax, yMin;
  if (metric.mark === "bar") {
    yMax = Math.max(0, ...allVals) * 1.15 || 1;
    yMin = Math.min(0, ...allVals) * 1.15;
  } else {
    const dataMax = Math.max(...allVals), dataMin = Math.min(...allVals);
    const pad = (dataMax - dataMin) * 0.15 || Math.abs(dataMax) * 0.1 || 1;
    yMax = dataMax + pad; yMin = dataMin - pad;
  }
  ...
  const axisY = (yMin <= 0 && yMax >= 0) ? y(0) : MT + plotH; // zero may be outside the range for an auto-scaled rate axis
  svg.appendChild(svgEl("line", { class: "axis-line", x1: ML, x2: W - MR, y1: axisY, y2: axisY }));
