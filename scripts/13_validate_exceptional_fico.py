"""
13_validate_exceptional_fico.py
...
Deliberately does not import pnl_utils.py / 06_pnl_rollup.py (re-implements
the bucketing rule from scratch) -- same "don't share the builder's blind
spot" discipline as 08_audit.py. Findings get written up in BUILD_LOG.md by
...
Three parts:
  ...
  2. Systematic outlier scan: cohort-level undiscounted 12Q CP/Account
     (independently recomputed, same method as 08_audit.py Check E) via
...
def run_hand_derivations(df):
    ...
    result.to_csv(OUT_DIR / "exceptional_fico_hand_derivation.csv", index=False)
    print(f"\nWrote {OUT_DIR / 'exceptional_fico_hand_derivation.csv'}")
    return result
...
def recompute_cp_per_account(df):
    ...
    08_audit.py's check_e_ltv_sanity (independently re-derived here too,
...
def run_outlier_scan(df):
    ...
    rates = pd.read_csv(OUT_DIR / "curve_rate_curves.csv")
...
def run_rewards_sanity_check(df):
    ...
    rates = pd.read_csv(OUT_DIR / "curve_rate_curves.csv")
    rewards = rates[rates["Line Item"] == "Rewards"]
    pooled_merchants = set(pd.read_csv(OUT_DIR / "pooled_merchants.csv")["pooled_merchant"])
    ...
    if pooled_merchants:
        print(f"\n  -> {len(pooled_merchants)} merchant(s) ({', '.join(sorted(pooled_merchants))}) use a single "
              "blended Rewards rate across ALL FICO tiers (thin-history pooling, see 04_curve_library.py's "
